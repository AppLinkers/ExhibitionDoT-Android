...
class ImageProcessor @Inject constructor(
    ...
    suspend fun uriToCompressedFile(uri: Uri, reqWidth: Int, reqHeight: Int): File? {
        val (suffix, compressFormat) = getSuffixAndCompressFormatByVersion()
        val tempFile = createTempImageFile(suffix = suffix)
        val downSampledBitmap = getDownSampledBitmap(uri, reqWidth, reqHeight) ?: return null
        val correctedBitmap = rotateBitmapIfRequired(downSampledBitmap, uri)
//        Log.i(TAG, "bitmap 크기: ${downSampledBitmap.byteCount / 1024}Kb")
        compressBitmapToFile(correctedBitmap, tempFile, compressFormat)
            .also { correctedBitmap.recycle() }
//        Log.i(TAG, "file 크기: ${tempFile.length() / 1024}Kb, 위치: ${tempFile.absoluteFile}")
        return tempFile
    }
    ...
    private fun getDownSampledBitmap(uri: Uri, reqWidth: Int, reqHeight: Int): Bitmap? {
    ...
    private suspend fun rotateBitmapIfRequired(bitmap: Bitmap, uri: Uri): Bitmap =
        withContext(Dispatchers.Default) {
            val inputStream = context.contentResolver.openInputStream(uri) ?: return@withContext bitmap
